...
class ProjectsBlock extends Component
{
    ...
    public function render()
    {
        ...
        if (!empty($this->project_ids)) {
            ...
            $isHomePage = request()->is('/'); // Assuming the home page URL is '/'
    
            // Determine the number of projects to fetch based on the page
            $projectLimit = $isHomePage ? 3 : PHP_INT_MAX; // Display 3 projects on the home page, all on the category page
            ...
            ])->whereIn('id', $this->project_ids)->take($projectLimit)->get();
        }
        $partner = PartnerModel::all();
    
        return view('livewire.pagebuilder.projects-block', compact('projects','partner'));
    }
